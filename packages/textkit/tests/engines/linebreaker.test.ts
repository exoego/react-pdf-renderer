...
describe('linebreaker', () => {
  ...
  test('should not add hyphens when hyphens option is "none"', () => {
    const linebreakerNoHyphens = linebreakerFactory({ hyphens: 'none' });
    ...
    const result = linebreakerNoHyphens(attributedString, [10]);

    // With hyphens: 'none', no hyphens should be added at line breaks
  ...
  test('should use custom hyphenate character when hyphenateCharacter is set', () => {
    const linebreakerCustomChar = linebreakerFactory({
      hyphenateCharacter: '・',
    });
    const attributedString = {
      string: 'Potentieel broeikasgasemissierapport',
    ...
    const result = linebreakerCustomChar(attributedString, [10]);

    // Custom hyphenate character should be used instead of '-'
    expect(result.map((line) => line.string)).toEqual([
      'Potentieel ',
      'broeikasgas・',
      'emissie・',
  ...
  test('should not add hyphens when hyphenateCharacter is empty string', () => {
    const linebreakerEmptyChar = linebreakerFactory({ hyphenateCharacter: '' });
    const attributedString = {
      string: 'Potentieel broeikasgasemissierapport',
      runs: [
        {
    ...
    const result = linebreakerEmptyChar(attributedString, [10]);

    // Empty hyphenate character means no hyphen at line breaks
    expect(result.map((line) => line.string)).toEqual([
      'Potentieel ',
      'broeikasgas',
      'emissie',
